'''
Usage:    
    $ python sparsity_prune.py --weights outputs\yolov5s_wd_adamw\weights\best.pt --data data\widerface.yaml --percent 0.2
'''
import argparse
import json
import os
...
def test_prune(data,
    ...
    # Directories
    opt.name += '_after_prune'
    save_dir = increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok)  # increment run
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir

    # Load model, need fuse=False
    ...
    for i, layer in model.named_modules():
        ...
        if isinstance(layer, nn.BatchNorm2d) and (layer not in ignore_bn_list):
                model_list[i] = layer

    # print("@Moyan model_list:", [layer for layer in model_list.keys()])
    # print("@Moyan ignore_bn_list:", ignore_bn_list)
    ...
    for bnname, bnlayer in model.named_modules():
        if isinstance(bnlayer, nn.BatchNorm2d):
            bn_module = bnlayer
            mask = obtain_bn_mask(bn_module, thre, device)
            if bnname in ignore_bn_list:
                # mask = torch.ones(bnlayer.weight.data.size()).cuda()
                mask = torch.ones(bnlayer.weight.data.size()).to(device)
    ...
    # =============================================================================================== #
    torch.save({"model": model}, save_dir / "orign_model.pt")
    model = pruned_model
    torch.save({"model":model}, save_dir / "orign_model_pruned.pt")
...
if __name__ == '__main__':
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally    
        ...
        test_prune(opt.data, opt.weights, opt.batch_size, opt.img_size, 
            opt.conf_thres, opt.iou_thres, opt.save_json, 
            opt.single_cls, opt.augment, opt.verbose,
            save_txt=opt.save_txt | opt.save_hybrid,
            save_hybrid=opt.save_hybrid,
